...
struct PriorityQueue<Element: Hashable, Priority: Comparable>: Collection {
    ...
    init(values: [Element], priorities: [Priority]) {
        heap = values
    ...
    mutating func add(_ value: Element, priority: Priority) {
        heap.append(value)
    ...
    mutating private func sink(_ index: Int) {
        var i = index
        while true {
            let left = 2 * i + 1
            let right = 2 * i + 2
    ...
    @discardableResult
    mutating private func remove(at index: Int) -> Element? {
        if isEmpty { return nil }
